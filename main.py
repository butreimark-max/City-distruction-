...
"""
Смотри, это моя карта - тут будут как перекроёстки, так и дороги в разные стороны + дороги Т-образные в разные стороны
Мне нужно придумать паттерн удобный, чтобы на моей карте было удобно генерировать разные объекты:
- Перекрёстки
- Дорогие вертикальные 
- Горизонтательные
- Т-образные (в разные стороны)
- Все остальные пустые нули - это будут дургие объекты (по типу домов и т.д.)

В чём сложность: в том, что один перекрёсток может заспавниться на строчке под номером 0, а второй на строчке под номером 1
И я не понимаю, как их можнео удобно заспавнить или связать, или куда дальше пойдёт дорога?

Мне нужно, чтобы ты максимально на простом языке (желательно с визаулизацией) объяснил мне и придумал
Удобный и простой алгоритм для генерации карты. НО ВАЖНО: не пиши за меня код, я просто хочу, чтобы ты мне "на пальцах" объяснил материал
